...
**Review Queue / Triage** — pending entity matches with accept/reject, per-source health that distinguishes error / never-run / stale / disabled, duplicate-candidate lineages ranked strongest-first (proposals only — nothing is auto-merged or dismissed), judge/human disagreement flags, and stale license facts.

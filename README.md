...
| Security-press RSS (The Record, BleepingComputer) | Stored — cyber-incident reporting for backfill (incident classification later) |
...
- **Ingestion layer** — a shared runner (per-source policy checks, TTL skips, run bookkeeping, idempotent native-id/content-hash dedupe, per-source error containment, single-writer lock) plus eleven live fetchers. Five feed classified, card-producing sources: SEC EDGAR submissions (8-K/10-K per watchlist CIK), Federal Register (FERC + TSA documents), press-wire RSS (PR Newswire, GlobeNewswire), NERC standards-page snapshots, and the ransomware.live victims feed (content-hash dedupe — no native id). Six are **store-only backfill** — no classification yet, so later stages classify against history instead of cold: the CISA KEV catalog, GDELT energy-sector news (rolling ~90-day DOC API window), the NVD CVE API (120-day-windowed, paged), EIA plant capacity records (paged v2, keyed), and security-press RSS (The Record, BleepingComputer — reputable cyber-incident reporting, guid-deduped, store-only and link-out only). A 12-month backfill (local — the database is gitignored and rebuildable) is stored and re-runs dedupe to zero.
...
- **Classification & scoring (rule-based MVP)** — a classifier framework (entity resolution with review-queue gating, parent rollup, deterministic signal ids, per-version bookkeeping so re-runs are incremental and rule changes reprocess history) plus five precision-first classifiers: leadership changes (8-K Item 5.02 + press-wire appointment grammar, security-relevant titles only), regulatory actions (Federal Register FERC/TSA rules with a required compliance-clock anchor; NERC standards-page diffs), and cyber incidents from three sources — SEC 8-K **Item 1.05** (the SEC's dedicated Material Cybersecurity Incidents item over the stored EDGAR backfill: one filing mints a **confirmed** `own_incident` card for the filer and a name-free, sector-wide `peer_incident` card), the **ransomware.live** leak-site tracker (each victim resolved by name → a watchlist match mints an `own_incident` account card, an off-list victim a name-free `peer_incident` sector card, both **unconfirmed early-warning** under the R10.5 evidence tiers; ambiguous/collision names go to the review queue and never auto-fire), and **company statements** (a watchlist company's own press-wire release disclosing a cyber incident → a **confirmed** `own_incident` card, resolved by name with the same review-queue gating; a strict verb+cyber-phrase grammar excludes fiduciary-duty "breach" notices and vendor threat reports — recall-latent until real breach releases land in the wire backfill). Incident cards carry an evidence tier and gate customer-facing outreach per R7.12 — an unconfirmed early-warning card is operator-only: it leads with a "no confirmation yet" verification-first warning, suppresses the outreach opener, and is kept out of account precision (R8.6). Scores follow `base_strength × 0.5^(age/half-life) × account_fit × scope_fit` with operator-tunable weights seeded from CSV; stale signals decay automatically. Every signal carries ranked evidence rows — nothing surfaces unsourced.
...
Later stages classify the remaining ingested incident source (security-press RSS, now stored; regulator incident notices are covered by the existing FERC/NERC enforcement path), add combo scoring, GDELT-based classification, and a hiring/macro-trend layer.

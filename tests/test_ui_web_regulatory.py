"""Regulatory Monitor tests for the FastAPI web UI (R8.4, R7.2, D8).

The TestClient port of tests/test_ui_regulatory_monitor.py: same hermetic
fixture (temp-file SQLite via apply_migrations, pointed at the app through
GRIDSIGNALS_DB, no network), same trust assertions carried across the framework
change. The data-layer suite (TestRegulatoryData) is retained verbatim — it
exercises data.regulatory_monitor directly and is framework-agnostic; the page
suite is re-expressed against the rendered HTML.
...
signals and with NO signals row referencing them (gating preserved); the
headline quotes the source's own text with no action verb; no signal fields
leak; nerc_pages snapshots shape correctly; the anchor flag is descriptive; a
malformed payload survives; the page renders a distinct list surface (not the
signal card), disclaims graduation, escapes upstream text, and shows an honest
...
class RegulatoryPageBase(RegulatoryTestBase):
    def setUp(self):
        super().setUp()
        self.client = TestClient(app)

    def get(self):
        resp = self.client.get("/regulatory")
        self.assertEqual(resp.status_code, 200)
        return resp.text

    def record_containing(self, dom, needle):
        """The single gs-record div block whose markup contains ``needle`` —
        lets a per-record assertion isolate one row (the source AppTest scoped
        the same way via the element's own value). Split on the record boundary
        so each block spans exactly one record's markup."""
        for block in dom.split('<div class="gs-record">')[1:]:
            if needle in block:
                return block
        self.fail(f"no gs-record block contained {needle!r}")


class TestRegulatoryPage(RegulatoryPageBase):
    def test_boot_smoke_200(self):
        self.assertEqual(self.client.get("/regulatory").status_code, 200)

    def test_renders_no_exception_and_chatter_shown(self):
        dom = self.get()
        # headline title is html-escaped in the DOM (';' survives; XSS guard)
        self.assertIn(CHATTER_TITLE, dom)
        # graduated + non-regulatory titles must NOT appear (gating preserved)
        self.assertNotIn(GRAD_TITLE, dom)
        self.assertNotIn(NONREG_TITLE, dom)

    def test_nav_and_chrome(self):
        dom = self.get()
        self.assertIn('href="/regulatory"', dom)
        self.assertIn("Regulatory Monitor", dom)

    def test_row_has_agency_meta_and_source_link(self):
        dom = self.get()
        # agency-prefixed headline, source-name meta line, and an http source link
        self.assertIn("Federal Energy Regulatory Commission notice:", dom)
        self.assertIn("Federal Register", dom)          # source_name in meta
        self.assertIn('href="http://fr/chatter"', dom)  # safe_source_url link

    def test_framing_disclaims_graduation(self):
        dom = self.get()
        # The load-bearing disclaimer sentence, with NOT emphasized as <strong>.
        self.assertIn(
            "These are raw Federal Register / NERC records,", dom)
        self.assertIn("<strong>NOT</strong>", dom)
    ...
    def test_chatter_record_has_no_severity_treatment(self):
        ...
        gs-record, never a gs-card with a severity score strip."""
        dom = self.get()
        self.assertIn('class="gs-record"', dom)
        # no signal-card wrapper, no severity band class on the rendered records
        self.assertNotIn('class="gs-card', dom)
        self.assertNotIn("sev-", dom)

    def test_source_link_only_http_https(self):
        """XSS guard: a non-http(s) source URL must NOT reach the DOM as a link."""
        _add_raw_event(self.conn, "re_js", "federal_register", "2026-08-14", {
            "title": "Sneaky record", "type": "Notice",
            "agencies": [{"name": "FERC"}]},
            url="javascript:alert(1)")
        _mark_classified(self.conn, "re_js")
        self.conn.commit()
        dom = self.get()
        self.assertIn("Sneaky record", dom)          # row still renders
        self.assertNotIn("javascript:alert", dom)    # but the scheme never links

    def test_upstream_title_html_escaped(self):
        """Autoescape XSS guard: raw HTML in an upstream title is escaped, not
        rendered as markup."""
        _add_raw_event(self.conn, "re_xss", "federal_register", "2026-08-15", {
            "title": "<script>alert('x')</script>", "type": "Notice",
            "agencies": [{"name": "FERC"}]},
            url="http://fr/xss")
        _mark_classified(self.conn, "re_xss")
        self.conn.commit()
        dom = self.get()
        self.assertNotIn("<script>alert('x')</script>", dom)
    ...
    def test_nerc_record_shows_no_compliance_clock_chip(self):
        ...
        _mark_classified(self.conn, "re_nerc")
        self.conn.commit()
        record = self.record_containing(self.get(), "CIPStandards.aspx")
        self.assertIn("page snapshot", record)
        # the always-true clock chip is suppressed for nerc_pages records
        self.assertNotIn("no compliance clock", record)
        self.assertIn(">chatter<", record)

    def test_empty_state_renders(self):
        # Remove the only chatter row so the page hits its honest empty state.
